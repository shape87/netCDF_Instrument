...
class StormOptions(StormData):
    ...
    def __init__(self):
        ...
        super(StormOptions).__init__(StormData)
        self.netCDF = {
...
#                      'Crest': None,
    ...
    def get_datasets(self):
        if self.sea_fname is not None:
            self.sds = open_url(self.sea_fname)
            
        if self.air_fname is not None:
            self.ads = open_url(self.air_fname)
            
        if self.wind_fname is not None:
            self.wds = open_url(self.wind_fname)
...
#             c_pressure[self.clip_query] = np.NaN
    ...
    def get_meta_data(self):
        if self.datum is None:
            self.datum = nc.get_geospatial_vertical_reference(self.sea_fname, self.sds)
            self.latitude = nc.get_variable_data(self.sea_fname,'latitude', self.sds)
            self.longitude = nc.get_variable_data(self.sea_fname,'longitude', self.sds)
            self.stn_station_number = nc.get_global_attribute(self.sea_fname, 'stn_station_number', self.sds)
            self.stn_instrument_id = nc.get_global_attribute(self.sea_fname, 'stn_instrument_id', self.sds)
            
    def get_air_meta_data(self):
        if self.air_latitude is None:
            
            if self.from_water_level_file == False:
                self.air_latitude = nc.get_variable_data(self.air_fname,'latitude', self.ads)
                self.air_longitude = nc.get_variable_data(self.air_fname,'longitude', self.ads)
                self.air_stn_station_number = nc.get_global_attribute(self.air_fname, 'stn_station_number', self.ads)
                self.air_stn_instrument_id = nc.get_global_attribute(self.air_fname, 'stn_instrument_id', self.ads)
            else:
                self.air_latitude = nc.get_variable_data(self.air_fname,'latitude', self.ads)
                self.air_longitude = nc.get_variable_data(self.air_fname,'longitude', self.ads)
                self.air_stn_station_number = nc.get_global_attribute(self.air_fname, 'stn_station_number', self.ads)
                self.air_stn_instrument_id = nc.get_variable_attr(self.air_fname, 'air_pressure', 'instrument_serial_number', self.ads)
            
    def get_wind_meta_data(self):
        if self.wind_latitude is None:
            
            try:
                self.wind_latitude = nc.get_variable_data(self.wind_fname,'latitude', self.wds)
                self.wind_longitude = nc.get_variable_data(self.wind_fname,'longitude', self.wds)
                self.wind_stn_station_number = nc.get_global_attribute(self.wind_fname, 'stn_station_number', self.wds)
            except:
