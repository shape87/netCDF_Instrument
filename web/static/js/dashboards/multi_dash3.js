...
//		        style: myStyle}).addTo(map);
	...
	var update_content = function(dash_object, data, change)
	{
		...
		case "water_level_graph":
			...
			//Get the appropriate graph data
			var graph_data = data[data.length-1];
	...
	var ajax_post = function(pairs, post_errors)
	{
		if (post_errors < 5)
		{
		    ...
		    for (var i = 0; i < pairs.length; i++ ){
		        sea_ids.push(stations[pairs[i].sea_id * 2].query);
		        air_ids.push(stations[pairs[i].air_id * 2].query);
		    }
			...
			formData.append('sea_file', sea_ids);
			formData.append('baro_file', air_ids);
