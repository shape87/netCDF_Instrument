...
class Stats(object):
    ...
    def manual_chi2_cdf(self, df, ci):
        1 / (2**(df/2) * self.factorial(df/2, True)) * np.trapz(df/2-1 * np.exp(ci/2)) 
    
    def factorial(self, number, start=False):
