...
$('.wind_expand').click(function(){
	...
	var ajax_post = function(pairs, post_errors, order)
	{
		if (post_errors < 5)
		{
			$.ajax({
				type: 'POST',
				url: window.location.href + 'single',
				data: {
	...
	{
		console.log('wpost errors ' + wind_post_errors);
		if (wind_post_errors < 5)
		{
			$.ajax({
				type: 'POST',
				url: window.location.href + 'wind',
				data: {
					start_time: $('#start_date').val(),
					end_time: $('#end_date').val(),
					daylight_savings: $('#dst').val(),
		...
		{
			console.log('error, didnt feel like trying');
		}
	}
